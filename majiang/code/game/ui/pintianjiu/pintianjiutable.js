...
gameclass.pintianjiutable.prototype.onChat = function(index,data){
    ...
    if(data.type < 4){
        ...
        if(data.type == 1){
            ...
            _node.addChild(bg, 1111);
            if (this.index == 0) {
                _node.setPositionX(talkPos.x);
                _node.setPositionY(talkPos.y);
...
gameclass.pintianjiutable.prototype.setRoomInfo = function(begin){
    ...
    if(begin) {
        ...
        ccui.helper.seekWidgetByName(_this.node,"btnZuobi").setVisible(false);
    }
    else if(_this.curround>0){
        _this.againready();
    }
};
gameclass.pintianjiutable.prototype.setRoomZuoBi = function(isVip){
    this.isZuoBi=isVip;
    ccui.helper.seekWidgetByName(this.node,"btnZuobi").setVisible(this.isZuoBi);
};

gameclass.pintianjiutable.prototype.zuobiend = function () {
    if(this.isZuoBi){
        this.panelzuobi.setVisible(false);
        gameclass.showText("您已选牌成功!!!");
    }
};

//显示作弊界面
gameclass.pintianjiutable.prototype.sendzuobi = function(cardArr){
    this.panelzuobi.setVisible(true);
    var _this=this;
    var leftpai=0; //牌的剩余次数
    var paitable=[];//牌对象
    var sendCard=[];//选择的手牌
    var painumber=this.ptjtype==0?4:2;
    for(var i=1;i<=4;i++){
        ccui.helper.seekWidgetByName(this.panelzuobi, "dian_"+i).setVisible(false);
    }
    ccui.helper.seekWidgetByName(this.panelzuobi, "paixing_1").setVisible(false);
    ccui.helper.seekWidgetByName(this.panelzuobi, "paixing_2").setVisible(false);
    if(this.ptjtype==1){
        ccui.helper.seekWidgetByName(this.panelzuobi, "show_3").setVisible(false);
        ccui.helper.seekWidgetByName(this.panelzuobi, "show_4").setVisible(false);
    }

    gameclass.createbtnpress(this.panelzuobi, "btnClose", function () {
        _this.panelzuobi.setVisible(false);
    });

    gameclass.createbtnpress(this.panelzuobi, "btnOk", function () {
        for(i=0;i<painumber;i++){
            if(!sendCard[i]||sendCard[i]==0){
                _this.panelzuobi.setVisible(false);
                return ;
            }
        }
        _this.mod_ptj.sendgamezuobi(sendCard);
        cc.log(sendCard)
    });
    var setdipai=function(tag,diannum){
        var showpai=ccui.helper.seekWidgetByName(_this.panelzuobi, "dian_"+tag);
        sendCard[tag-1]=diannum;
        if(diannum==0){
            showpai.setVisible(false);
        }else{
            showpai.setVisible(true);
            var strname = "ptj_dianimg_"+diannum+".png";
            showpai.loadTexture(strname,ccui.Widget.PLIST_TEXTURE);
            if(diannum==4){              //特殊处理一下
                showpai.setScale(1.2,1.1);
                showpai.setRotation(180);
            }else{
                showpai.setScale(1,1);
            }
        }

        var sprmminame = ccui.helper.seekWidgetByName(_this.panelzuobi, "paixing_1");
        sprmminame.setVisible(false);
        var minname = _this.showgroupname([sendCard[0],sendCard[1]],true);//是否返回报错图片毕十
        if(minname&&sendCard[0]&&sendCard[1]&&sendCard[0]!=0&&sendCard[1]!=0){
            sprmminame.setVisible(true);
            sprmminame.initWithSpriteFrameName(minname+".png");
        }

        var sprmaxname = ccui.helper.seekWidgetByName(_this.panelzuobi, "paixing_2");
        sprmaxname.setVisible(false);
        var maxname = _this.showgroupname([sendCard[2],sendCard[3]],true);//是否返回报错图片毕十
        if(maxname&&sendCard[2]&&sendCard[3]&&sendCard[2]!=0&&sendCard[3]!=0){
            sprmaxname.setVisible(_this.ptjtype==0);
    ...
    for(var i=1;i<22;i++){
        var paicount=i<=11?2:1;

        var objpai=""+i+paicount;

        var pai={};
        pai.obj=ccui.helper.seekWidgetByName(this.panelzuobi, "imgDi_"+objpai);
        pai.obj.setVisible(cardTimes[i]>0);
        pai.obj.setTag(i);
        pai.count=cardTimes[i];
        pai.dianshu=i;
        paitable.push(pai);
        gameclass.createbtnpress(this.panelzuobi, "imgDi_"+objpai, function (sender) {
            var idx=sender.getTag();
            if(leftpai>=painumber||paitable[idx-1].count<=0) return ;
            leftpai++;
            paitable[idx-1].count--;
            paitable[idx-1].obj.setVisible(paitable[idx-1].count>0);
            var setpaitag;
            ...
            if(!setpaitag)setpaitag=leftpai ;
            setdipai(setpaitag,idx)
        });
    }

    for(var i=1;i<=painumber;i++){
        var dian=ccui.helper.seekWidgetByName(this.panelzuobi,"dian_"+i);
        dian.setTag(i);
        gameclass.createbtnpress(this.panelzuobi, "dian_"+i, function (sender) {
            var idx=sender.getTag();
            if(sendCard[idx-1]==0||!sendCard[idx-1]) return;
            if(leftpai<=0) return ;
            paitable[sendCard[idx-1]-1].count++;
            paitable[sendCard[idx-1]-1].obj.setVisible(paitable[sendCard[idx-1]-1].count>0);
            setdipai(idx,0);
            leftpai--;
        });
    }
...
gameclass.pintianjiutable.prototype.share = function(){
    ...
    gameclass.mod_platform.invitefriend(
        shareText,
        gameclass.download,
        "秦皇棋牌-" +  this.mod_ptj.roominfo.roomid +"-拼天九");
};
...
gameclass.pintianjiutable.prototype.init = function(){
    ...
    this.btnZuobi =ccui.helper.seekWidgetByName(this.node, "btnZuobi");
    this.btnZuobi.setVisible(this.isZuoBi);
    this.panelzuobi =ccui.helper.seekWidgetByName(this.node, "panelZuoBi");
    this.panelzuobi.setVisible(false);
    ...
    gameclass.createbtnpress(this.node, "btnZuobi", function () {
        _this.mod_ptj.sendgethand();
        // _this.sendzuobi();
    });
...
gameclass.pintianjiutable.prototype.dianchiState = function(){
    //var dianchiState =  ccui.helper.seekWidgetByName(this.node, "dianchi");
    //if (dianchiState==null)
    //{
    //    return;
    //}
    //var reftime = function () {
    //    var result = gameclass.mod_platform.getBatteryInfo();
    //    if (result)
    //    {
    //        var result1 = JSON.parse(result);
    //        dianchiState.setPercent(result1.curLevel);
    //    }
    //};
    //reftime();
    //var func = cc.repeatForever(cc.sequence(cc.delayTime(120),cc.callFunc(function(){
    //    reftime();
    //})));
    //dianchiState.runAction(func);
};
...
gameclass.pintianjiutable.prototype.showtablecard = function(msgdata) {
    ...
    for(var i = 0; i < 32; i++){
        ...
        if(_this.tablecards[i] > 0) {
            ...
            var cardchild = new ccui.ImageView(res["ptj_dianimg_"+_this.tablecards[i]]);
        ...
        var animation = function(){
            if(!flag){
                ...
                for(var j = 0; j < len; j++){
                    ...
                    //var cardchild = new cc.Sprite();
                    //cardchild.initWithSpriteFrameName(strname);
                    var cardchild = new ccui.ImageView(res["ptj_dianimg_"+viewcardsnum[j]]);
                    cardchild.setAnchorPoint(cc.p(0,0));
                    //cardchild.loadTexture(strname,ccui.Widget.PLIST_TEXTURE);
                    var ind = 0;
...
gameclass.pintianjiutable.prototype.touziFunc = function ( touzinums )
{
    ...
    //var spineBoy = sp.SkeletonAnimation.createWithJsonFile(res.ptjsaizijson, res.ptjsaiziatlas);
    //spineBoy.setAnimation(19, "yaoshaizi", false);
    var spineBoy = sp.SkeletonAnimation.createWithJsonFile(res.tousaizi_json, res.tousaizi_skeleton);
    spineBoy.setAnimation(19, 'w', false);
    spineBoy.setCompleteListener(removeSprite);
...
gameclass.pintianjiutable.prototype.autoGroupscard = function(groupmax,groupmin) {
    ...
    for(var mn = 0; mn < _this.mycardsgroup.length; mn++){
        if(_this.mycardsgroup[mn].flag&&_this.mycardsgroup[mn].cardnode) {
            _this.mycardsgroup[mn].flag = false;
            _this.mycardsgroup[mn].prechoose = 0;
                        ...
                        for(var m=0; m < msgdata.info.length; m++){
                            ...
                            if(chair1){
                                ...
                                for(var n = 0; n < tempcard.length; n++){
                                    ...
                                    var cardchild1 = new ccui.ImageView(res["ptj_dianimg_"+tempcard[n]]);
                                    cardchild1.setAnchorPoint(cc.p(0,0));
