...
gameclass.ddzhutable = gameclass.baseui.extend({
    ...
    setmod: function (_mod) {
        ...
        btn_layer = new gameclass.btn_setLayer(_this.node, _this.game,_this.mod_ddzhu);
        this.node.addChild(btn_layer);
        btn_layer.setTurn(_this.RoomMin);
    ...
    getSexFromuid: function (uid) {
        for (var i = 0; i < 3; i++) {
            if (this.players[i].data.uid == uid) {
                var sex = this.players[i].data.sex;
                if (sex == 1 || sex == 2) {
                    return sex - 1;
                } else {
                    return 0;
...
gameclass.ddzhutable.prototype.onchat = function (data) {
    ...
    var sex = this.getSexFromuid(data.uid);
    for (var i = 0; i < g_chatstrDdz.length; i++) {
        if (g_chatstrDdz[i] == data.chat) {
            if(sex == 0){
                mod_sound.playeffect(g_music["tell" + i + "_W"], false);
            }else{
                mod_sound.playeffect(g_music["tell" + i + "_M"], false);
            }
        }
    }
...
gameclass.ddzhutable.prototype.ongameDDZbegin = function (data) {
    ...
    this.RoomMin++;
    this.mod_ddzhu.currentTurn = this.RoomMin;
    if (_this.RoomMin != 1) {
        _this.resetDDZNext();
    }
...
gameclass.ddzhutable.prototype.ongamebets = function (data, noadd) {
    ...
    if (!noadd) {
        if (data.bets > _this.bets) {
            if (sex = 0) {
                mod_sound.playeffect(g_music["Woman_Rob_" + data.bets], false);//不抢
                // staticFunction.playUserTalk("Woman_Rob", sex);
            } else {
                mod_sound.playeffect(g_music["Man_Rob_" + data.bets], false);//不抢
                // staticFunction.playUserTalk("Man_Rob", sex);
            }
            _this.updateDifenView(data);
        } else {
            if (sex = 0) {
                mod_sound.playeffect(g_music["Woman_NoOrder"], false);//不抢
            } else {
...
gameclass.ddzhutable.prototype.ongameddzstep = function (stepData, noadd) {
    ...
    if (stepData.cards.length > 0) {
        ...
        if (!noadd) {
            if (sex = 0) {
                staticFunction.playUserTalk("Woman_buyao", sex);
            } else {
                staticFunction.playUserTalk("Man_buyao", sex);
            }
...
gameclass.ddzhutable.prototype.alarmRes = function (alarmuid, index, alarmcount) {
    ...
    if (this.alarmUid[index]) {
        ...
        this.scheduleOnce(function () {
            var sex = this.getSexFromuid(this.players[index].data.uid);
            if (alarmcount == 1) {
                if(sex == 0){
                    mod_sound.playeffect(g_music["Woman_one_card"], false);
                }else{
                    mod_sound.playeffect(g_music["Man_one_card"], false);
                }
            } else {
                if(sex == 0){
                    mod_sound.playeffect(g_music["Woman_two_card"], false);
                }else{
...
gameclass.ddzhutable.prototype.resultend = function () {
    if (this.isDDZend) {
        ...
        var myScore;
        for(var i = 0;i<this.ddzenddata.info.length;i++){
            if(this.ddzenddata.info[i].uid == this.game.modmgr.mod_login.logindata.uid){
                myScore = this.ddzenddata.info[i].score;
            }
            if(this.ddzenddata.info[i].score > maxScore){
                maxScore = this.ddzenddata.info[i].score;
            }
        }
        if(myScore >= maxScore){
...
gameclass.ddzhutable.prototype.showEndCard = function (endCard, index) {
    ...
    if (index == 1) {
        for (var i = tmpCardData.length - 1; i >= 0; i--) {
            ...
            /*tmpCardData.length * 20 / 2 - (20 * i) - 70*/
        ...
        for (var i = 0; i < tmpCardData.length; i++) {
            ...
            sp.setRotation(90);
            sp.setPosition(0, 100);
            /*tmpCardData.length * 20 / 2 - (20 * i) - 60*/
            sp.runAction(cc.moveBy(0.8, 0, -17 * i + 70));
            sp.setScale(0.6);
            this.players[index].cards.addChild(sp);
        }
...
gameclass.ddzhutable.prototype.initUserData = function (data) {
    for (var i = 0; i < data.info.length; i++) {
        this.refplayerDDZinfo(data.info[i].card, i);
...
gameclass.ddzhutable.prototype.share = function () {
    ...
    gameclass.mod_platform.invitefriend(str,
        gameclass.download,
        "斗地主-" + ddz_mode + "-" + this.mod_ddzhu.roomnum);

};
gameclass.ddzhutable.prototype.checkSafe = function (people) {
    this.safeLayer.checkSafe(people);

    var wanfaArr = StaticData.getRoomSetArrFromParam(this.mod_ddzhu.roominfo.type, this.mod_ddzhu.roominfo.param1, this.mod_ddzhu.roominfo.param2, 1);
    var ss = wanfaArr;
};
...
gameclass.ddzhutable.prototype.init = function () {
    ...
    //var btn_layer = new gameclass.btn_setLayer(_this.node, _this.game,_this.mod_ddzhu);
    //this.node.addChild(btn_layer);
...
gameclass.ddzhutable.prototype.playRes = function (soundType, value, sex) {
    switch (soundType) {
        case this.tool_cardType.CARDTYPE.danzhang:
            if(sex == 0){
                mod_sound.playeffect(g_music["Woman_" + value], false);
            }else{
        ...
        case this.tool_cardType.CARDTYPE.duizi:
            if(sex == 0){
                mod_sound.playeffect(g_music["Woman_dui" + value], false);
            }else{
        ...
        case this.tool_cardType.CARDTYPE.sanzhang:
            if(sex == 0){
                mod_sound.playeffect(g_music["Woman_tuple" + value], false);
            }else{
        ...
        case this.tool_cardType.CARDTYPE.zhadan:
            if(sex == 0){
                mod_sound.playeffect(g_music["Woman_zhadan"], false);
            }else{
        ...
        case this.tool_cardType.CARDTYPE.sandaiyi:
            if(sex == 0){
                mod_sound.playeffect(g_music["Woman_sandaiyi"], false);
            }else{
        ...
        case this.tool_cardType.CARDTYPE.sandaier:
            if(sex == 0){
                mod_sound.playeffect(g_music["Woman_sandaiyidui"], false);
            }else{
        ...
        case this.tool_cardType.CARDTYPE.liandui:
            if(sex == 0){
                mod_sound.playeffect(g_music["Woman_liandui"], false);
            }else{
        ...
        case this.tool_cardType.CARDTYPE.shunzi:
            if(sex == 0){
                mod_sound.playeffect(g_music["Woman_shunzi"], false);
            }else{
        ...
        case this.tool_cardType.CARDTYPE.sidaierdan:
            if(sex == 0){
                mod_sound.playeffect(g_music["Woman_sidaier"], false);
            }else{
        ...
        case this.tool_cardType.CARDTYPE.sidaierdui:
            if(sex == 0){
                mod_sound.playeffect(g_music["Woman_sidailiangdui"], false);
            }else{
        ...
        case this.tool_cardType.CARDTYPE.shuangfei:
            if(sex == 0){
                mod_sound.playeffect(g_music["Woman_feiji"], false);
            }else{
        ...
        case this.tool_cardType.CARDTYPE.huojian:
            if(sex == 0){
                mod_sound.playeffect(g_music["Woman_wangzha"], false);
            }else{
 ...
 *  初始发牌
 */
gameclass.ddzhutable.prototype.deal = function (_cards) {
    this.handCard = [];
...
gameclass.ddzhutable.prototype.reset = function () {
    ...
    this.RoomMin = this.mod_ddzhu.minStep || 0;
...
gameclass.ddzhutable.prototype.refplayerDDZinfo = function (_cards, index) {
    var tmpCards = [];
...
gameclass.ddzhutable.prototype.userLineOut = function (index, data) {
    ...
    this.players[index].off_line.setLocalZOrder(this.players[index].icon.getChildren().length - 1);
};
