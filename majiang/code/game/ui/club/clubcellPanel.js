...
gameclass.clubcellPanel = gameclass.baseui.extend({
    ...
    show: function () {
        this.node = this.game.uimgr.createnode(res.julebu_clubcells, true, 1, true);
    ...
    wxInvite: function () {
        var _this = this;
        var clubId = this.game.uimgr.uis["gameclass.clubmanger"]._clubData.id;
        gameclass.createbtnpress(this.node, "inviteFriendBtn", function () {
            gameclass.mod_platform.invitefriend("", "http://www.hbyouyou.com/down/android/name/poker", StaticData.GAME_NAME + "，快来俱乐部一起玩吧，俱乐部ID[" + clubId + "]");
        });
        gameclass.createbtnpress(this.node, "inviteCircleBtn", function () {
            gameclass.mod_platform.invitefriendSpoons("", "http://www.hbyouyou.com/down/android/name/poker", StaticData.GAME_NAME + "，快来俱乐部一起玩吧，俱乐部ID[" + clubId + "]");
        });
    },
