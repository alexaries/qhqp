...
gameclass.uimgr.prototype.showload = function(show){
    ...
    //
    //     var loadingAnim = sp.SkeletonAnimation.createWithJsonFile(res.loadingJson, res.loadingAtlas);
...
gameclass.uimgr.prototype.showui = function (name, close, parent, screen) {
    ...
    if (screen == null || screen == 0) {
        // cc.log("swicthscreen0");
        //cc.view.setDesignResolutionSize(640, 1136, cc.ResolutionPolicy.SHOW_ALL);
        gameclass.mod_platform.swicthscreen(0);
    } else if (screen == 1) {
        // cc.log("swicthscreen1");
        //cc.view.setDesignResolutionSize(1136, 640, cc.ResolutionPolicy.SHOW_ALL);
        gameclass.mod_platform.swicthscreen(1);
    }

    var ui = this.uis[name];
    if (ui != null && close == false) {
    ...
    // var ui = null;
    // if(name == "gameclass.hallui"){
    //     ui = new gameclass.hallui();
    // }else if(name == "gameclass.loginui"){
    //     ui = new gameclass.loginui();
    // }else if(name == "gameclass.createroomui"){
    //     ui = new gameclass.createroomui();
    // }else if(name == "gameclass.hlgc.Table"){
    //     ui = new gameclass.hlgc.Table();
    // }else if(name == "gameclass.FightSetView"){
    //     ui = new gameclass.FightSetView();
    // }else if(name == "gameclass.msgboxui"){
    //     ui = new gameclass.msgboxui();
    // }else{
    //     cc.log("name====="+name)
    // }
    this.gettmepparent().addChild(ui);
    this.uis[name] = ui;
    this.uis[name].setgame(this.game);
    this.uis[name].show();
    return this.uis[name];
};

gameclass.uimgr.prototype.createnode = function (filename, resize, centernodename, showType) {
    ...
    if (ret != null) {
        // ret.setOpacity(128);
        // var ani = cc.sequence(cc.fadeTo(0, 128),cc.fadeTo(0.3, 255),cc.callFunc(function(sender){
        //
        // }));
        // ret.runAction((ani));
...
gameclass.uimgr.prototype.closeui = function (name, close) {
    if (this.uis[name] != null) {
        ...
        //uimgr.uis[name].removeFromParent();
        this.uis[name] = null;
    }
};
...
gameclass.uimgr.prototype.getui = function (name) {
    return this.uis[name];
};
