    ...
    this.mywebsocket.setonmsgfunc(function (ws,data) {
        ...
        switch (data.msghead){
            ...
            case "gameddzinfo":
                ...
                if( _this.roominfo.person[0].uid == _this.game.modmgr.mod_login.logindata.uid)
                {
                    _this.isHouse = true;
                }
                else
                ...
                if(data.msgdata.begin) {
                    ...
                    _this.mainUI.RoomMin = _this.minStep;
            ...
            case "gamedealer":
                ...
                var sex = _this.view.getSexFromuid(data.msgdata.uid);
                if(sex == 0){
                    mod_sound.playeffect(g_music["Woman_Rob_" + data.msgdata.bets], false);//不抢
                    // staticFunction.playUserTalk("Woman_Rob", sex);
                }else{
                    mod_sound.playeffect(g_music["Man_Rob_" + data.msgdata.bets], false);//不抢
...
gameclass.mod_ddzhu.prototype.updateroominfo = function(roominfo) {
    ...
    for (var i =0;i < this.roominfo.person.length; i++){
        ...
        if(i == 0){
            this.bankerUid = this.roominfo.person[i].uid;
        }
    }
...
gameclass.mod_ddzhu.prototype.dissmissroom = function(num) {
    var data = {type: num};
