...
gameclass.mod_platform.getAddress = function () {
    var jsonStr = gameclass.mod_platform.getmapinfo();
...
gameclass.mod_platform.getmapinfo = function () {
    if (!cc.sys.isNative) {
        ...
        if (gameclass.test == "true") { //! 高版本不开启地图定位
            return "";
        }
...
gameclass.mod_platform.invitefriend = function (txt, roomId, title) {
    game.modmgr.mod_center.isclickPYQ = false;
...
gameclass.mod_platform.invitefriendSpoons = function (txt, roomId, title) {
    game.modmgr.mod_center.isclickPYQ = true;
    if (!cc.sys.isNative) {
        gameclass.mod_platform.wxshare(txt, roomId, title);
    }
    else {
...
gameclass.mod_platform.savescreen = function (callfunc, ondrawover) {
    ...
    if (!cc.sys.isNative) {
        ...
        var savescene = function () {
            ...
            cc.log("hpptPos----->" + "platform" + "||data=" + jsonData);
            PostURL("http://game.hbyouyou.com/youyou.image/Index/uploadBase64Image", jsonData, function (url) {
        ...
        var act = cc.sequence(cc.delayTime(1), cc.callFunc(function () {
...
gameclass.mod_platform.outPutAndroidLog = function(str){
    if (cc.sys.os == cc.sys.OS_WINDOWS) {

    }else if(cc.sys.os == cc.sys.OS_ANDROID){
        jsb.reflection.callStaticMethod("org/cocos2dx/javascript/JsCall", "outPutAndroidLog", "(Ljava/lang/String;)V", str.toString());
    }else if(cc.sys.os == cc.sys.OS_IOS){
    }
    else
    {
...
gameclass.mod_platform.onlogin = function (code) {
    ...
    game.modmgr.mod_login.wxlogin(code);
...
gameclass.mod_platform.onMessaegResp = function () {
    game.modmgr.mod_center.weixinShareOK();
};
...
gameclass.mod_platform.wxshare = function (txt, roomId, title) {
    if (window.wx) {
        ...
        cc.log(roomId);
        cc.log(title);
        var appId = "wxe29f6e4668070234";
        var url = "https://open.weixin.qq.com/connect/oauth2/authorize?" +
            "appid=" + appId + "&" +
            "redirect_uri=http%3a%2f%2fcentre.hbyouyou.com%2fUser%2floginNiuniu" + "&" +
            "response_type=code" + "&" +
...
gameclass.mod_platform.getBattery = function (_num) {
    //if(game.uimgr.uis["gameclass.hallui"]){
    //    game.uimgr.uis["gameclass.hallui"].changeShow(_num);
    //}
    if (cc.sys.os == cc.sys.OS_WINDOWS) {
        ...
        if (game.uimgr.uis["gameclass.hallui"]) {
            return _num;
            //return jsb.reflection.callStaticMethod("org/cocos2dx/javascript/JsCall", "getBattery", "(Ljava/lang/String;)V");
        }
    } else if (cc.sys.os == sys.OS_IOS) {
